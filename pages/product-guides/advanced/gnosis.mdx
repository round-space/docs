...
When creating a bounty using a Safe, you will notice that creating a bounty only requires a signature. Once completed, you'll see a prompt on the bounty page to Complete the process by approving the transaction in your Gnosis safe transaction queue. While this is pending, the bounty status is "Creating" and no claims are allowed on the bounty.

## Rewarding a Bounty

Submissions on a bounty created using a Safe doesn't affect the submitters experience. Once a submission is ready to be rewarded, a signature is required and the reward amounts are automatically released. This makes a Safe bounty more gas efficient than a bounty from a regular Eth wallet like Metamask.
